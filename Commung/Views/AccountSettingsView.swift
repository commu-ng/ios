    ...
    var body: some View {
        NavigationView {
            Form {
                ...
                // Data Section
                Section(header: Text(NSLocalizedString("account.data", comment: ""))) {
                    Button {
                        Task {
                        ...
                        HStack {
                            Label(NSLocalizedString("account.export_data", comment: ""), systemImage: "arrow.down.doc.fill")
                            Spacer()
            ...
            .alert(NSLocalizedString("account.log_out", comment: ""), isPresented: $showingLogoutConfirmation) {
                Button(NSLocalizedString("action.cancel", comment: ""), role: .cancel) {}
                Button(NSLocalizedString("account.log_out", comment: ""), role: .destructive) {
                    Task {
            ...
            .alert(NSLocalizedString("account.delete_account", comment: ""), isPresented: $showingDeleteConfirmation) {
                Button(NSLocalizedString("action.cancel", comment: ""), role: .cancel) {}
...
class AccountSettingsViewModel: ObservableObject {
    ...
    func requestDataExport() async {
        ...
        do {
            try await AccountService.shared.requestDataExport()
            successMessage = NSLocalizedString("account.export_success", comment: "")
        } catch {
            errorMessage = "Failed to request export: \(error.localizedDescription)"
        }
    ...
    func logout() async {
        do {
            ...
            // App will handle navigation after logout
        } catch {
    ...
    func deleteAccount() async {
        ...
        do {
            ...
            // App will handle navigation after account deletion
        } catch {
